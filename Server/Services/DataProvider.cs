...
namespace Server.Services
{
  public class DataProvider
  {
    ...
    public void DeleteTour(string tour_description, int year, Cassandra.TimeUuid tourId)
    {
      //Session.Execute($"delete from bus_tracker.tour WHERE tour_description = '{tour_description}' and year = {year} and tourId = {tourId} if exists");
      Session.Execute($"delete from bus_tracker.vechiclefuel using timestamp {DateTimeOffset.Now.ToUnixTimeMilliseconds()}  where  tourId = {tourId} and reading_time = '2022-01-03 12:24:31.994000+0000'  if exists");
      Session.Execute($"delete from bus_tracker.vechiclelocation where  tourId = {tourId}  if exists");
      Session.Execute($"delete from bus_tracker.vechicleidling where  tourId = {tourId}  if exists");
    ...
    {
      return Session.Execute($"SELECT * FROM bus_tracker.vechiclefuel WHERE tourId = {tourId} ");
    }
